...
interface User {
  ...
  full_name: string;
  email: string;
  properties?: string[]; // For multi-property owners
}
...
const PropertyOwnerDashboard: React.FC = () => {
  ...
  const [currentYearData, setCurrentYearData] = useState<YearlyMetrics | null>(null);
  ...
  const checkAuth = useCallback(async () => {
    try {
      ...
      // Fetch all properties for this user (same username = same owner)
      fetchUserProperties(data.user.username);
    } catch (err) {
  ...
  const fetchUserProperties = useCallback(async (username: string) => {
    try {
      // Fetch all properties owned by users with same username
      const response = await fetch(
        `${phpApiUrl}?action=user_properties&username=${username}`,
        { credentials: 'include' }
      );
      const data = await response.json();
      ...
      if (data.properties && data.properties.length > 0) {
        ...
        setSelectedProperty(data.properties[0]); // Select first property by default
      }
    } catch (err) {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      // Check if property has 180-day limit
  ...
  }, [user, selectedProperty, phpApiUrl]);

  const fetch180DayLimit = useCallback(async () => {
    if (!user || !selectedProperty || !has180DayLimit) return;
    ...
    try {
      ...
      // Find data for this specific property
      if (data.properties && data.properties.length > 0) {
  ...
  }, [user, selectedProperty, has180DayLimit, metricsApiUrl]);
  ...
  const fetchRoomsData = useCallback(async (year: number, month: number) => {
    ...
    try {
      ...
      // Determine which room endpoint to use based on property name
  ...
  }, [user, selectedProperty, metricsApiUrl]);

  const fetchCurrentYearData = useCallback(async () => {
    if (!user || !selectedProperty) return;

    try {
      const currentYear = new Date().getFullYear();
      const response = await fetch(
        `${phpApiUrl}?action=yearly&property=${selectedProperty}&year=${currentYear}`,
        { credentials: 'include' }
      );
      const data = await response.json();
      setCurrentYearData(data);
    } catch (err) {
      console.error('Failed to fetch current year data:', err);
    }
  }, [user, selectedProperty, phpApiUrl]);
  ...
  }, [user, selectedProperty, ytdYear, phpApiUrl]);

  const fetchMonthlyMetrics = useCallback(async () => {
    if (!user || !selectedProperty) return;

    try {
      // Fetch monthly data from yearly endpoint
      const response = await fetch(
      ...
      if (data && data.monthly_data && data.monthly_data[selectedMonth]) {
        ...
        // If it's a hostel, also fetch room-level data
  ...
  }, [user, selectedProperty, phpApiUrl, selectedMonth, selectedMonthYear, fetchRoomsData]);
  ...
  }, [user, selectedProperty, phpApiUrl, selectedYear]);
  ...
  }, [user, selectedProperty, phpApiUrl, compareYear1, compareYear2]);
  ...
  }, [user, selectedProperty, fetchSummary, fetchCurrentYearData, fetchMonthlyMetrics]);

  useEffect(() => {
    if (user && selectedProperty && activeTab === 'overview') {
      fetchMonthlyMetrics();
    }
  }, [user, selectedProperty, selectedMonth, selectedMonthYear, activeTab, fetchMonthlyMetrics]);

  useEffect(() => {
    if (user && activeTab === 'yearly') {
      fetchYearlyMetrics();
    }
  }, [user, selectedYear, activeTab, fetchYearlyMetrics]);

  useEffect(() => {
    if (user && activeTab === 'compare') {
      fetchComparison();
    }
  }, [user, compareYear1, compareYear2, activeTab, fetchComparison]);

  useEffect(() => {
    if (user && selectedProperty && activeTab === '180_limit' && has180DayLimit) {
      fetch180DayLimit();
    }
  }, [user, selectedProperty, activeTab, has180DayLimit, fetch180DayLimit]);

  useEffect(() => {
    if (user && selectedProperty && activeTab === 'overview') {
      fetchYtdYearData();
    }
  }, [user, selectedProperty, ytdYear, activeTab, fetchYtdYearData]);

  useEffect(() => {
    if (user && selectedProperty && activeTab === 'occ_record') {
      fetchOccRecord();
    }
  }, [user, selectedProperty, activeTab, occStartDate, occEndDate, fetchOccRecord]);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>{selectedProperty || summary?.property_name} Dashboard</h1>
      ...
      <div style={styles.content}>
        ...
        {activeTab === 'overview' && summary && (
          <div>
            ...
            {/* Monthly Room Performance Table (for hostels) - Selected Month/Year */}
                ...
                <div style={{
                  ...
                  <p style={{
                    ...
                    {limitData.property.is_over_limit ? (
                      '⚠️ You have exceeded the 180-day limit! No more bookings can be accepted for this fiscal year.'
                    ) : limitData.property.status === 'critical' ? (
                      '⚠️ Less than 30 days remaining. Please monitor your bookings carefully.'
